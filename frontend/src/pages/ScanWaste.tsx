...
import { Button, Card, EmptyState, Field, Loading, Note, Spinner, useAsync } from '../components/ui';
...
export function ScanWaste() {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      if (err instanceof AiUnavailableError) {
        // The backend reports 503 when Gemini is not configured or unreachable. Nothing is
        // fabricated — the user classifies the item manually instead.
  ...
  return (
    <div className="grid cols-2">
      <div>
        <Card title="1 · Photograph the item">
          <label className="dropzone" htmlFor="scan-file">
            ...
            {previewUrl ? (
              ...
              <>
                ...
                <div className="strong">Tap to choose a photo</div>
          ...
          <div className="btn-row" style={{ marginTop: 14 }}>
            <Button type="button" onClick={analyze} disabled={!file || analyzing || activeCategories.length === 0}>
              {analyzing ? <Spinner onPrimary /> : null}
          ...
          {analysis ? (
            <div style={{ marginTop: 14 }}>
              <Note tone="success" icon="✨">
                <strong>{analysis.item}</strong> looks like{' '}
                <strong>{analysis.categoryName}</strong>
                {analysis.confidence !== null ? ` (confidence ${percent(analysis.confidence)})` : ''}.
                {analysis.aiModel ? <div className="small muted">Model: {analysis.aiModel}</div> : null}
              </Note>
            </div>
          ) : null}
        </Card>

        <Card title="2 · Confirm the details">
          ...
          <Field label="Waste category" htmlFor="category" hint="Choose the closest match">
            <select id="category" value={categoryId} onChange={(e) => setCategoryId(e.target.value)} disabled={categories.loading}>
          ...
          <div className="inline wrap" style={{ gap: 18, marginBottom: 14 }}>
            <div className="check">
              <input
                id="recyclable"
                type="checkbox"
          ...
          {confidence !== null ? (
            <div className="small muted" style={{ marginBottom: 12 }}>
              Saved as an AI-assisted scan (confidence {percent(confidence)}).
            </div>
          ) : (
            <div className="small muted" style={{ marginBottom: 12 }}>
              Saved as a manual scan.
            </div>
          )}
      ...
      <div>
        <Card title="Recent scans" action={<Button type="button" className="ghost small" onClick={recent.reload}>Refresh</Button>}>
          {recent.error ? <Note tone="error">{recent.error}</Note> : null}
          {recent.loading ? (
            <Loading />
          ) : (recent.data?.content.length ?? 0) === 0 ? (
            <EmptyState icon="🗂️" title="No scans recorded yet">
            ...
            <div className="list">
              {activeCategories.map((category) => (
                <div className="list-item" key={category.id}>
                  <span className="swatch" style={{ background: category.colorHex ?? '#6B705C', width: 12, height: 12, borderRadius: 4, marginTop: 6 }} aria-hidden="true" />
                  <div className="grow">
